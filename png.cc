...
void WritePngToMemoryGrayAlpha(size_t w, size_t h, const unsigned char *dataVA, std::vector<unsigned char> *out) {
    out->clear();
    png_structp p = png_create_write_struct(PNG_LIBPNG_VER_STRING, NULL, NULL, NULL);
    ASSERT_EX(p, "png_create_write_struct() failed");
    TPngDestructor destroyPng(p);
    ...
    std::vector<unsigned char*> rows(h);
    for (size_t y = 0; y < h; ++y)
        rows[y] = (unsigned char*)dataVA + y * w * 2;
    png_set_rows(p, info_ptr, &rows[0]);
    png_set_write_fn(p, out, PngWriteCallback, NULL);
    png_write_png(p, info_ptr, PNG_TRANSFORM_IDENTITY, NULL);
}
